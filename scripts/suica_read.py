...
# This code is copied from https://github.com/m2wasabi/nfcpy-suica-sample
...
class HistoryRecord(object):
    def __init__(self, data):
        ...
        self.sequence_id = self.get_sequence_id(row_le[9], row_le[10], row_le[11])
    ...
    @classmethod
    def get_sequence_id(cls, v1, v2, v3):
    ...
    if isinstance(tag, nfc.tag.tt3.Type3Tag):
        try:
            ...
            for i in range(num_blocks):
                ...
                "=== %02d ===" % i
                print
                "端末種: %s" % history.console
                print
                "処理: %s" % history.process
                print
                "日付: %02d-%02d-%02d" % (history.year, history.month, history.day)
                print
                "入線区: %s-%s" % (history.in_station.company_value, history.in_station.line_value)
                print
                "入駅順: %s" % history.in_station.station_value
                print
                "出線区: %s-%s" % (history.out_station.company_value, history.out_station.line_value)
                print
                "出駅順: %s" % history.out_station.station_value
                print
                "残高: %d" % history.balance
                print
                "BIN: "
                print
